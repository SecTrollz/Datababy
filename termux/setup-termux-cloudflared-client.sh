...
pkg update -y
# Note: cloudflared has no Termux package — it's downloaded as a raw binary
# below, the same way frpc is.
pkg install -y wget tar openssl-tool tmux cronie termux-services

ARCH=$(uname -m)
case "$ARCH" in
    aarch64) FRP_ARCH="arm64"; CFD_ARCH="arm64" ;;
    armv7l|armv8l) FRP_ARCH="arm"; CFD_ARCH="arm" ;;
    x86_64) FRP_ARCH="amd64"; CFD_ARCH="amd64" ;;
...
echo "[2/5] Downloading frpc ($FRP_ARCH) and cloudflared ($CFD_ARCH)..."
...
CLOUDFLARED_BIN="$FRP_DIR/cloudflared"
wget -q "https://github.com/cloudflare/cloudflared/releases/latest/download/cloudflared-linux-${CFD_ARCH}" -O "$CLOUDFLARED_BIN"
...
if ! tmux has-session -t cfd 2>/dev/null; then
    tmux new-session -d -s cfd "$CLOUDFLARED_BIN access tcp --hostname $TUNNEL_HOSTNAME --url 127.0.0.1:$LOCAL_FORWARD_PORT"
    sleep 2
fi
